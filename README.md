...
> 标题左侧 `⋮` 拖动柄移动面板位置，底部 `═` 拖动柄调整面板高度；两者均记忆，双击复位。
...
| 特性 | 说明 |
| --- | --- |
| 🟢 实时状态 | 运行中（🟦 蓝色渐变扫光方块，与 DSH 会话框指示器同款 + 秒表）、完成、失败、已打断、令牌上限、已拒绝 |
| 🃏 卡片化列表 | 每个子代理一张圆角卡片；「打开对话」在右侧，状态与耗时在第二行 |
| 🌲 树形缩进 | 孙代子代理卡片向右缩进 |
| 🔙 一键返回 | 进入子代理会话后，面板出现「← 主会话」按钮 |
| 🖐 自由摆放 | 标题左侧拖动柄移动面板，位置自动记忆；双击复位 |
| 📏 高度可调 | 底部拖动柄调整面板高度，高度自动记忆；双击复位 |
...
| 状态 | 含义 |
| --- | --- |
| 🟦 运行中 | 正在执行，蓝色渐变扫光方块（与 DSH 会话框指示器同款）+ 实时秒表 |
| 🟢 完成 | 面板实时见证其成功结束，显示耗时 |
...
**面板位置和高度会记住吗？** 会。拖动后写入 localStorage，刷新页面 / 重启浏览器后恢复；双击拖动柄恢复默认。
...
完整变更历史见 [CHANGELOG.md](./CHANGELOG.md)。当前版本 **0.2.0**（与 `package.json` 对齐）。
